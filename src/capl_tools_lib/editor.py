...
class CaplEditor:
    ...
    def __init__(
        self,
        file_mngr: Optional[CaplFileManager] = None,
    ...
    def remove_elements(self, elements: List[CAPLElement]) -> None:
        ...
        for el in elements_sorted:
            logger.info(
                f"Removing element {el.name} at lines {el.start_line}-{el.end_line}"
            )
