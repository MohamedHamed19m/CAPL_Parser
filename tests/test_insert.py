...
""")
    ...
    result = processor.insert(
        location="after:testFunction", element_type="TestCase", code_string=new_code
    )
...
def test_insert_with_file(tmp_path):
    ...
    processor = CaplProcessor(test_file)
    result = processor.insert(
        location="after:testFunction", element_type="TestCase", source=snippet_file
    )
...
def test_insert_requires_source_or_code(tmp_path):
    ...
    with pytest.raises(ValueError, match="Must provide either"):
        processor.insert(location="after:something", element_type="TestCase")
